...
import { Droplets, Thermometer, Gauge, Activity, TrendingUp, TrendingDown, Minus } from "lucide-react";
import { format } from "date-fns";
import { cn } from "@/lib/utils";

const ZONE_COLORS = ["#22c55e", "#f59e0b", "#3b82f6", "#a855f7"];
const ZONE_COLORS_MUTED = ["#22c55e40", "#f59e0b40", "#3b82f640", "#a855f740"];
...
export default function Sensors() {
  ...
  // Per-zone recent trend (last 5 readings)
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3">
                ...
                <div className="text-xl font-bold text-amber-400 tabular-nums">
                  {sensors.data?.temperature?.toFixed(1) ?? "—"}
              ...
              <div className="rounded-lg bg-blue-500/10 border border-blue-500/20 p-3">
                ...
                <div className="text-xl font-bold text-blue-400 tabular-nums">
                  {sensors.data?.humidity?.toFixed(0) ?? "—"}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={tankData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                    ...
                    <ReferenceLine
                      ...
                      stroke="#ef4444"
                      strokeDasharray="4 4"
                      label={{ value: "Critical 20%", fill: "#ef4444", fontSize: 9, position: "insideTopRight" }}
                    />
                    <Area
                      type="monotone"
                      dataKey="Tank"
      ...
      <Card className="border-card-border">
        <CardContent className="p-4 flex items-center gap-3">
          <div className={cn(
            ...
            sensors.data?.waterLoggingDetected ? "bg-red-500/15" : "bg-primary/15"
          )}>
            <Activity className={cn("w-4 h-4", sensors.data?.waterLoggingDetected ? "text-red-400" : "text-primary")} />
          </div>
          <div>
            <div className="text-sm font-semibold">Water Logging Sensor</div>
            <div className="text-xs text-muted-foreground">Field waterlogging detection via ESP32 digital input</div>
          </div>
          <Badge
            className="ml-auto"
            variant={sensors.data?.waterLoggingDetected ? "destructive" : "outline"}
          >
            {sensors.data?.waterLoggingDetected ? "WATERLOGGING DETECTED" : "Clear — No Waterlogging"}
          </Badge>
        </CardContent>
      </Card>
    </div>
